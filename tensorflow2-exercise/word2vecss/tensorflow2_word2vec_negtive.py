...
def sample(group, context_length):
    ...
    for uid, moviesLs in group.items():
        i, j = 0, context_length
        total = 0
        while j < len(moviesLs):
            sample = context_sample(moviesLs[i:j])
            i += 1
            j += 1
            total += len(sample)
            for s in  sample:
...
if __name__ == '__main__':
    ratings = pd.read_csv(r'C:\Users\hanwenlinPersonal\Desktop\tensorflow2-turtorial\tensorflow2-exercise\ml-1m\ratings.dat', sep='::', names=['user_id', 'movie_id', 'rating', 'unix_timestamp'])
    movies = pd.read_csv(r'C:\Users\hanwenlinPersonal\Desktop\tensorflow2-turtorial\tensorflow2-exercise\ml-1m\movies.dat', sep='::', names=['movie_id', 'title', 'genres'])
